  ...
  <div>
    <div class="flex-1 p-3" v-if="selectedLayers.length">
      <div
        ...
        class="group rounded-md bg-white p-3 shadow-sm dark:bg-gray-700"
      >
        <p class="text-gray-800 dark:text-gray-200">
          {{ selectedLayers.length }} produto(s) selecionado(s)
        </p>
        <div
          v-for="layer in selectedLayers"
          :key="layer.id"
          class="relative mb-2 flex items-center gap-2"
        >
            ...
            <div class="flex flex-col w-full">
              <div class="flex flex-col items-center gap-3 p-1">
                ...
                <div class="flex flex-col text-center">
                  ...
                  <div
                    class="grid grid-cols-2 gap-2 text-xs text-gray-500 dark:text-gray-400 mb-3"
                  >
                    <div>
                      <span class="font-medium">EAN:</span> {{ layer.product.ean }}
                    </div>
                    <div>
                      <span class="font-medium">Frentes:</span> {{ layer.quantity || 0 }}
                    </div>
                  </div>
                </div>
                <div
                  v-if="
                    getStockInfo(layer.product?.ean) ||
                    getProductAnalysis(layer.product?.ean)
                  "
                ...
                >
                  ...
                  <div class="grid grid-cols-2 gap-3">
                    <!-- Classificação ABC -->
                    <div
                      v-if="getProductAnalysis(layer.product?.ean)"
                    ...
                    <!-- Estoque Alvo -->
                    <div
                      v-if="getStockInfo(layer.product?.ean)"
                      class="bg-white dark:bg-gray-800 rounded-md p-3 text-center"
                    >
                      <div class="text-xs text-gray-500 dark:text-gray-400 mb-1">
                        Estoque Alvo
                  ...
                  >
                    <div class="bg-white dark:bg-gray-800 rounded-md p-2 text-center">
                      ...
                      <div class="text-sm font-bold text-green-600 dark:text-green-400">
                        {{
                          (
                            (getStockInfo(layer.product?.ean)?.serviceLevel || 0) * 100
                          ).toFixed(1)
                        }}%
                      </div>
                    </div>
                    <div class="bg-white dark:bg-gray-800 rounded-md p-2 text-center">
                      ...
                      <div
                        ...
                        :class="
                          getStockInfo(layer.product?.ean)?.highVariability
                      ...
                      >
                        {{
                          (
                            (getStockInfo(layer.product?.ean)?.variability || 0) * 100
                          ).toFixed(1)
                        }}%
                      </div>
                    </div>
                ...
                <!-- Dimensões -->
                <div
                  class="w-full bg-gray-50 dark:bg-gray-800 rounded-lg p-3 mb-3"
                  v-else
                >
                  <h5
                    ...
                    <div class="grid grid-cols-2 gap-3">
                      <div class="bg-white dark:bg-gray-800 rounded-md p-2 text-center">
                        ...
                        <div class="text-sm font-bold text-green-600 dark:text-green-400">
                          R$
                          {{
                            formatCurrency(layer.product.summary.sales_total_value || 0)
                          }}
                        </div>
                      </div>
                      <div class="bg-white dark:bg-gray-800 rounded-md p-2 text-center">
                  ...
                  <div class="grid grid-cols-3 gap-2">
                    ...
                    <div class="bg-white dark:bg-gray-800 rounded-md p-2 text-center">
                      ...
                      <div
                        class="text-sm font-bold"
                        :class="
                          getMarginColor(layer.product.summary.margem_percentual || 0)
                        "
                      >
                ...
                >
                  ...
                  <EditProduct
                    :product="getProductForEdit(layer.product)"
                  ...
                  <AlertConfirm
                    ...
                    message="Tem certeza que deseja atualizar a imagem deste produto?"
                    confirmButtonText="Atualizar"
                    cancelButtonText="Cancelar"
                    :isDangerous="true"
                    @confirm="updateProductImage"
                    @cancel="cancelDelete"
                    :record="layer"
                  >
                    <svg
      ...
      <div v-else class="p-3 text-center text-gray-500">Carregando detalhes...</div>
    </div>
    <div v-else class="p-3 text-center text-gray-500">Nenhum produto selecionado.</div>
  </div>
</template>
...
const updateProductImage = async (layer: Layer) => {
  const productId = layer.product?.id;
  if (!productId) {
    console.error("No product associated with the layer");
    return;
  }
  try {
    const response = await apiService.post(`/products/${productId}/update-image`, {
      ean: layer.product?.ean,
    }); 
    const { image_url } = response;
    if (!image_url) {
      throw new Error("URL da imagem não retornada pelo servidor");
    }
    // Atualiza o layer local
    layer.product!.image_url = image_url;
    
    // Atualiza o store para refletir a mudança em toda a gôndola
    editorStore.updateProductImage(productId, image_url);
