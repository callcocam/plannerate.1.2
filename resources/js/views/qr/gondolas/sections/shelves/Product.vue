...
// Variáveis para controle de duplo toque
let lastTouchTime = 0;
let touchTimeout: NodeJS.Timeout | null = null;

/**
* Manipula o duplo clique no produto para mostrar informações detalhadas
*/
const handleProductDoubleClick = (event: MouseEvent) => {
...
/**
* Manipula o duplo toque no mobile
*/
const handleProductTouch = (event: TouchEvent) => {
event.preventDefault();
event.stopPropagation();
...
// Se o tempo entre toques for menor que 300ms, é duplo toque
if (timeDiff < 300 && timeDiff > 0) {
    // Cancelar o timeout do toque simples se existir
    if (touchTimeout) {
        clearTimeout(touchTimeout);
        touchTimeout = null;
    }
    
    // Emitir evento de duplo toque
    emitProductInfo();
    lastTouchTime = 0; // Reset para evitar triplo toque
} else {
    // Primeiro toque - aguardar o segundo
    lastTouchTime = currentTime;
    
    // Timeout para resetar se não houver segundo toque
...
/**
* Emite as informações do produto
*/
const emitProductInfo = () => {
// Emitir evento com informações detalhadas do produto
const productInfo = {
    id: props.product?.id || '',
    name: props.product?.name || 'Produto sem nome',
    ean: props.product?.ean || 'EAN não informado',
    width: props.product?.width || 0,
    height: props.product?.height || 0,
    depth: props.product?.depth || 0,
    facing: props.product?.facing || props.layer?.quantity || 0,
    imageUrl: imageUrl.value,
    codigo_erp: props.product?.codigo_erp || 'Não informado'
};

// Emitir evento para o componente pai
if (typeof window !== 'undefined') {
    window.dispatchEvent(new CustomEvent('product-double-click', {
        detail: productInfo
    }));
}
};
...
if (productRef.value) {
    ...
    productRef.value.addEventListener('dblclick', handleProductDoubleClick);
    productRef.value.addEventListener('touchend', handleProductTouch);
} else {
...
onUnmounted(() => {
if (productRef.value) {
    productRef.value.removeEventListener('click', handleProductClick);
    productRef.value.removeEventListener('dblclick', handleProductDoubleClick);
    productRef.value.removeEventListener('touchend', handleProductTouch);
}
