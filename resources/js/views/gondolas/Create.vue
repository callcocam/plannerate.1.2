    ...
    <Dialog :open="isOpen">
        <DialogPersonaCloseContent
            ...
            <div class="border-b p-4 dark:border-gray-700">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <DialogTitle class="text-xl font-semibold dark:text-gray-100">{{ stepTitles[currentStep] }}
                        </DialogTitle>
                        <!-- Description updates based on current step -->
                        <DialogDescription class="dark:text-gray-300">{{ stepDescriptions[currentStep] }}
                        </DialogDescription>
                    </div>
                </div>
                ...
                <div class="mb-2 mt-3 flex items-center">
                    <template v-for="(step, index) in stepTitles" :key="index">
                        <div class="flex h-8 w-8 flex-none items-center justify-center rounded-full text-sm font-medium"
                            :class="{
                        ...
                        <div v-if="index < stepTitles.length - 1" class="mx-2 h-1 flex-1"
            ...
            <div class="flex-1 overflow-y-auto p-4 dark:bg-gray-800">
                ...
                <KeepAlive>
                    <component :is="stepComponents[currentStep]" :form-data="formData" :errors="errors"
            ...
            <div class="flex justify-between border-t bg-white p-4 dark:border-gray-700 dark:bg-gray-800">
                ...
                <Button v-if="currentStep < stepTitles.length - 1" @click="nextStep"
                ...
                <Button v-else @click="submitForm" :disabled="isSending"
                    class="dark:bg-primary dark:text-primary-foreground dark:hover:bg-primary/90">
...
const submitForm = async () => {
    ...
    const payload = {
        ...
        section: {
            ...
            shelf_config: { // Group shelf configurations
    ...
    try {
        ...
        // Close the modal (implicitly done by navigation, but emit to be sure)
        emit('update:open', false);
        emit('close');
