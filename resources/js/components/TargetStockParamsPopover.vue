...
function updateServiceLevel(classification: string, value: number) {
  const index = serviceLevels.value.findIndex(
    (sl: ServiceLevel) => sl.classification === classification
  );
  if (index !== -1) {
...
function updateCoverageDays(classification: string, value: number) {
  const index = replenishmentParams.value.findIndex(
    (rp: Replenishment) => rp.classification === classification
  );
  if (index !== -1) {
...
function handleCalculate() {
  ...
  setTimeout(() => {
    ...
    window.dispatchEvent(
      new CustomEvent("execute-target-stock-analysis", {
        detail: {
...
<template>
  <div class="flex flex-col gap-2 mb-2">
    ...
    <div class="grid grid-cols-2 gap-4">
      ...
      <div class="space-y-4">
        ...
        <div
          ...
          class="flex flex-col"
        >
          <label class="text-xs font-medium mb-2"
            >Classe {{ level.classification }}</label
          >
          <SmallInput
            v-model="level.level"
            type="number"
      ...
      <div class="space-y-4">
        ...
        <div
          v-for="param in replenishmentParams"
          :key="param.classification"
          class="flex flex-col"
        >
          <label class="text-xs font-medium mb-2"
            >Classe {{ param.classification }}</label
          >
          <SmallInput
    ...
    <div class="flex justify-end mt-2 gap-2">
      <Button @click="handleCalculate" variant="default" :disabled="isCalculating">
