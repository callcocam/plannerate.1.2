...
watch(() => props.weights, (val) => {
    weights.value = val;
}, { deep: true }); // Observar profundamente para mudanças internas no objeto
...
function atualizarCampo(field: keyof Weights | keyof Thresholds, valor: number, type: 'weights' | 'thresholds') {
    if (type === 'weights') {
        // Garantir que o campo é válido para weights antes de atribuir
        if (field === 'quantity' || field === 'value' || field === 'margin') {
         ...
         // Garantir que o campo é válido para thresholds antes de atribuir
        if (field === 'a' || field === 'b') {
...
<template>
    <div class="flex flex-col gap-2 mb-2">
        ...
        <div class="flex justify-end mt-2 gap-2">
            ...
            <Button @click="handleCalculate" variant="default"
